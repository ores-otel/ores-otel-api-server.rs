#![forbid(unsafe_code)]
//! Serving the four avenues from `ores-otel-api-server`.
//!
...
//! |---|------|---------------------|
//! | 1 | `direct_read` | none: the web server reads Postgres itself through `ores-lib-core` |
...
///
/// Bind `ORES_OTEL_API_MTLS_ADDR` and pass the listener. One task per
...
/// Open the JetStream context for avenue 4 from `ORES_OTEL_NATS_URL`.
///
/// # Errors
