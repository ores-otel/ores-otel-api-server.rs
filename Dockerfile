...
#
# Multi-stage image for ores-otel-api-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t ores-otel-api-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" ores-otel-api-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=ores-otel-api-server-target-${TARGETARCH},sharing=locked \
...
COPY --from=build "/usr/local/bin/ores-otel-api-server" "/usr/local/bin/ores-otel-api-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/ores-otel-api-server
...
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/ores-otel-api-server"]
