...
# Prefer linux/arm64: docker buildx build --platform linux/arm64 -t ores-otel-api-server:dev .
# ores-sops: https://github.com/ORESoftware/ores-sops — decrypt at run, never build.

FROM rust:1.90-bookworm AS build
WORKDIR /src
COPY . .
RUN cargo build --release --bin ores-otel-api-server \
...
COPY --from=build "/src/target/release/ores-otel-api-server" "/usr/local/bin/ores-otel-api-server"
