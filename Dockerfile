...
#
# ores-otel-api-server — production container image.
#
...
#   docker buildx build --platform linux/arm64 -t ghcr.io/ores-otel/ores-otel-api-server:dev --load .
#
...
#     -t ghcr.io/ores-otel/ores-otel-api-server:dev --load .
#
#   # OCI image index with provenance and SBOM attestations:
#   docker buildx build --platform linux/arm64 --push \
#     --provenance=true --sbom=true \
#     --output type=image,oci-mediatypes=true,name=ghcr.io/ores-otel/ores-otel-api-server:dev .
#
#   `just docker-build` / `just docker-run` / `just docker-push` wrap all three.
#
# DEPENDENCIES — this crate resolves several dependencies from git remotes and its
# .zpkg.toml declares none, so there is no zed-pkg stage: cargo resolves the
# graph directly. Private git entries are covered by the optional BuildKit
# token secret shown above, never by --build-arg.
#
...
#
# OBSERVABILITY (ores-otel) — the app exports OTLP in-process. The default
# collector is dd-otel-collector.observability.svc.cluster.local over
# HTTP/protobuf on :4318; the wrong port silently drops spans. Do NOT EXPOSE
# 4317/4318 here — this image does not receive OTLP, it emits it. The
# *-sidecar.rs image is a separate loopback probe helper on 127.0.0.1:9090:
# do not bake it into this image and do not publish :9090.
...
COPY --from=builder --chown=65532:65532 /usr/local/bin/ores-otel-api-server /usr/local/bin/ores-otel-api-server
ENV ORES_OTEL_API_BIND=0.0.0.0:8080 \
    OTEL_SERVICE_NAME=ores-otel-api-server \
    OTEL_EXPORTER_OTLP_ENDPOINT=http://dd-otel-collector.observability.svc.cluster.local:4318 \
    RUST_LOG=info
EXPOSE 8080
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/ores-otel-api-server"]
...
COPY --from=builder /usr/local/bin/ores-otel-api-server /usr/local/bin/ores-otel-api-server
...
#     ghcr.io/ores-otel/ores-otel-api-server:dev
#
...
CMD ["/usr/local/bin/ores-otel-api-server"]
